...
- ADR-0012 因此提出将初始 RRF rank 和固定 reranker rank 做等权二次
  ...
  no-fallback 行为，尚需新的不可变 158 条回归验证；
- Terra 120 条生成评测、生产发布和 Web/OpenIM/Telegram E2E 尚未
  完成。

## 7. 最终结论

当前实现未通过检索质量门槛，不能进入全量检索、生成评测或生产切换。
安全与完整性指标通过只说明 ACL-first 边界保持正确，不代表检索质量
合格。ADR-0011 的标题投影已经完成并测得明显改善，但 158 条回归仍差
3 条 Top-5 命中才达到其前置门槛。只读诊断已经证明主要可修复损失来自
reranker 对初始融合顺序的破坏。下一步是验证 ADR-0012 的有限排序改动，
并用新提交、新二进制摘要、新根和新服务只运行一次 158 条回归；不得降低
门槛或覆盖既有失败报告。在完整门槛、Node2 三通道和故障注入完成前，
本报告不声称企业知识库生产验收通过。
