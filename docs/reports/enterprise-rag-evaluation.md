...
- 评测器已在本地改为每请求 4 个问题、最多 2 个请求并发，并保持问题和
  ...
  clean build 在新的不可变 Node2 回归根完成全部 158 条评测；
- 不可变回归报告 SHA-256 为
  `8c17ab204b8d7714b0d26dab950f69dfba3133f2b73890b9ae75995340117a29`。
  schema、投影、样本数、ACL、旧版本、来源和 checksum 门禁均通过；
  Recall@5 为 `0.582278`，低于 `0.60`，Recall@10 为 `0.797468`，
  MRR 为 `0.393028`，nDCG@10 为 `0.455133`，因此服务以真实质量失败
  终止，未启动全量评测；
- 剩余 32 条 Top-10 失败中，31 条是 `single_document`，1 条是
  `numeric`；26 条来自高度同构的 runbook `触发条件`，5 条来自 SOP
  `办理步骤`。该分布说明标题投影有明显收益，但同模板候选的最终排序仍
  不足；
- 独立只读排序诊断已正常完成。报告 SHA-256 为
  `a7b330024e4522a45e4c4b99eefe2cf30a2f4fc08f6ae00904bb9ca1ca550e1d`，
  158 个 QA ID 和每个 QA 内的金标 Chunk ID 唯一，排名约束全部成立，
  且不包含 question、answer、title、content、excerpt 或 score 字段。
  候选池缺失 4 条，初始 RRF Top-10 命中 131 条；固定 reranker 挽救了
  13 条初始 Top-10 外结果，但又把 18 条初始 Top-10 结果降到 Top-10
  之外，最终形成 92 条 Top-5、34 条 rank 6-10 和 32 条 Top-10 miss；
- ADR-0012 因此提出将初始 RRF rank 和固定 reranker rank 做等权二次
  reciprocal-rank fusion。该方案不改变 ACL、候选上限、模型、门槛或
  no-fallback 行为；
- 新的不可变 158 条回归已通过：报告 SHA-256 为
  `9b8ba74789115780e81880677db42889f24e51d07284c7d158c96e6ec9e1d46f`，
  Recall@5 `0.664557`、Recall@10 `0.867089`、MRR `0.431846`、
  nDCG@10 `0.506764`，ACL/旧版本泄漏为 0，来源/checksum 为 1.0；
- `ac8b945` 固定工具已准备新的不可变全量根并通过重复只读校验。
  1,120 条检索服务只启动一次，当前尚未生成原子检索报告；生成和
  finalization 仍被前置门禁阻断；
- Terra 120 条生成评测、生产发布和 Web/OpenIM/Telegram E2E 尚未
  完成。

## 7. 最终结论

当前全量检索质量门槛尚未得到新报告验证，不能进入生成评测或生产切换。
ADR-0012 的有限排序改动已经通过 158 条前置回归，因此允许进行一次新的
全量检索评测；它不等于全量门槛已经通过。当前应只等待原子 1,120 条
检索报告，不得重启服务、运行生成或覆盖既有证据。在完整门槛、Node2
三通道和故障注入完成前，本报告不声称企业知识库生产验收通过。
