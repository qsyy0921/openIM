...
Current model-route change: generation is now fixed to `gpt-5.6-luna` through Windows-loopback `POST /v1/responses`. Local model discovery, direct Responses, local Worker candidate, and temporary Node2-to-Windows Worker forwarding are verified. The earlier DeepSeek OpenIM and Telegram Runs are historical architecture evidence only; migration `0029`, the permanent loopback SSH tunnel, and fresh dual-channel E2E remain required before the new route is marked Node2-verified.
