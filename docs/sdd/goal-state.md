...
| Enterprise RAG evaluation | node2_ranking_diagnostic_running | the original full 1,120-case report remains failed at Recall@5 `0.768269`; the title-aware 158-case regression missed its Recall@5 `0.60` gate at `0.582278`; read-only diagnostic commit `3f18580` and binary SHA-256 `fe54d49e...67293f` are running once from independent root `enterprise-rag-ranking-diagnostic-3f18580` | wait for the sanitized candidate/fusion/reranker rank report without restarting the service; propose a bounded ranking ADR only after measured attribution |
