...
| Enterprise RAG evaluation | node2_ranking_diagnostic_verified_local_fix_pending | the immutable sanitized report SHA-256 is `a7b33002...50e1d`; all 158 cases validate with no forbidden content fields; candidate-pool loss is 4, initial fusion Top-10 is 131, raw reranker Top-5/Top-10 is `92/126`, and the reranker demotes 18 initial Top-10 cases while rescuing 13; ADR-0012 is proposed | verify the bounded equal-weight retrieval/reranker rank fusion locally, then clean-build and run one new immutable 158-case regression |
...
- On 2026-07-26, the immutable read-only ranking diagnostic completed normally
  ...
  and diff checks pass locally. No full evaluation, production switch,
  generation request, migration, or channel action was started.
