...
| Fixed Responses generation | node2_verified | permanent loopback tunnel, real Node2 candidate, authorized ACL-RAG, revoked ACL, explicit no-match abstention, citation persistence, and cleaned OpenIM plus Telegram round trips passed on `responses3` | preserve the fixed route and repeat after generation-boundary changes |
...
| Telegram channel E2E | node2_verified | update `96338391` produced one published Outbox event, one successful `gpt-5.6-luna` Run with four citations, one sent delivery with an external message ID, `1|1|1` cardinality, and post-test binding cleanup `0|0` | repeat with a new isolated binding after Telegram or generation changes |
