...
func run() error {
	...
	role := flag.String("role", "", "platform_admin, agent_admin, or knowledge_admin")
	flag.Parse()
	enabled := false
	...
	defer pool.Close()
	if err := identity.NewPostgresStore(pool).SetMemberRole(ctx, strings.TrimSpace(*tenantID), strings.TrimSpace(*actorMemberID), strings.TrimSpace(*memberID), strings.TrimSpace(*role), enabled); err != nil {
		return err
	}
	fmt.Println("updated")
	return nil
